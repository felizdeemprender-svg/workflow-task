...
exports.generalChat = exports.onTaskStatusFinalized = exports.processAICommand = exports.getDailyBriefing = exports.adminUpdateUser = exports.adminCreateUser = exports.botResponse = exports.onUserUpdate = exports.syncCompany = void 0;
 ...
 */
exports.syncCompany = functions.https.onRequest(async (req, res) => {
 ...
 */
exports.botResponse = functions.runWith({ secrets: ["DEEPSEEK_API_KEY"] }).firestore
    .document("tasks/{taskId}/chat/{messageId}")
    .onCreate(async (snapshot, context) => {
    const message = snapshot.data();
    if (!message || !message.text.includes("@bot"))
        return;
...
exports.adminUpdateUser = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (phone)
            updates.phone = phone;
        if (accessibleAreas)
            updates.accessibleAreas = accessibleAreas;
 ...
 */
exports.getDailyBriefing = functions.runWith({ secrets: ["DEEPSEEK_API_KEY"] }).https.onCall(async (data, context) => {
    if (!context.auth)
 ...
 */
exports.processAICommand = functions.runWith({ secrets: ["DEEPSEEK_API_KEY"] }).https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError("unauthenticated", "Auth required");
    const { command } = data;
    if (!command)
    ...
    const DEEPSEEK_API_KEY = process.env.DEEPSEEK_API_KEY;
    if (!DEEPSEEK_API_KEY)
        throw new functions.https.HttpsError("failed-precondition", "AI missing API key");
    try {
        // Fetch company areas
        const companyId = userData.company_id;
        const companyDoc = await db.collection("companies").doc(companyId).get();
        const companyData = companyDoc.data();
        const availableAreas = (companyData === null || companyData === void 0 ? void 0 : companyData.areas) || ["General", "Ventas", "Logística", "Soporte"];
 ...
 */
exports.onTaskStatusFinalized = functions.runWith({ secrets: ["DEEPSEEK_API_KEY"] }).firestore
    ...
    const wasCompletedNow = (before.status !== "Finalizado" && after.status === "Finalizado") ||
    ...
    if (wasCompletedNow) {
        ...
        if (!userDoc && after.assignedEmail) {
            ...
            if (!userSnapshotOriginal.empty)
                userDoc = userSnapshotOriginal.docs[0];
        }
        if (!userDoc) {
 ...
 */
exports.generalChat = functions.runWith({ secrets: ["DEEPSEEK_API_KEY"] }).https.onCall(async (data, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError("unauthenticated", "Auth required");
